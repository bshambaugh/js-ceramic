...
import * as nist_weierstrass from '../../nist-weierstrass/src/index'
import { base64urlPoint } from '../../nist-weierstrass/src/index'
...
export function pubKeyBytesToXY(pubKeyBytes: Uint8Array) : base64urlPoint  {
  if(!nist_weierstrass.nist_weierstrass_common.testUint8Array(pubKeyBytes)) {
    throw new TypeError('input must be a Uint8Array');
  }
  const publicKeyHex = nist_weierstrass.nist_weierstrass_common.pubKeyBytesToHex(pubKeyBytes);
  const bytesCount = publicKeyHex.length / 2;

  // raw p-256 key
  if(bytesCount == 64) {
     return nist_weierstrass.nist_weierstrass_common.publicKeyToXY(publicKeyHex);
   }
  ...
  if(bytesCount == 65) {
   if(publicKeyHex.slice(0,2) == '04') {
     ...
     return nist_weierstrass.nist_weierstrass_common.publicKeyToXY(publicKey);
   }
  }
  ...
  if(bytesCount == 33) {
   if(publicKeyHex.slice(0,2) == '03' || publicKeyHex.slice(0,2) == '02') {
     ...
     const point = nist_weierstrass.secp256r1.ECPointDecompress(publicKey);
      return nist_weierstrass.nist_weierstrass_common.publicKeyIntToXY(point);
    }
  }
